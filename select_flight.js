...
async function show_flight(new_input_from, new_input_to, new_input_depart_date, new_input_return_date) {
    try {
        ...
        depart_flight_data.forEach(data => {
        ...
        return_flight_data.forEach(data => {
            const element = document.createElement("div");
            element.innerHTML = `
                ...
                <button class="btn btn-success choose-flight-btn flight-detail-items-i" id="select-btn">${data["return_cost"]} Baht</button>
            `;
